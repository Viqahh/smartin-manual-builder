...
|---|---|---|
| PRD-OQ-001 | **Styling approach.** ~~`docs/DEPENDENCIES.md` names shadcn/ui + Tailwind for Phase 1, but Phase 1 shipped a bespoke `app/globals.css` with design tokens and no Tailwind utilities. Which is authoritative going forward?~~ **RESOLVED (Phase 2): the bespoke `app/globals.css` token stylesheet is authoritative.** Tailwind v4 is `@import`-ed for its base reset/preflight only — no utility classes in components, no shadcn. Confirmed at 8B-11. See `docs/PHASE_2.md` §"PRD-OQ-001" and `docs/PHASE_8.md` §8B-11. | ~~Phase 2~~ Decided |
...
| PRD-OQ-003 | **Auth route group.** ~~Architecture folder tree uses `app/(auth)/login/`; Phase 1 shipped `app/login/`. Reconcile to which?~~ **RESOLVED (Phase 2): keep `app/login/`** (no `(auth)` group); `/login` + `/no-membership` at app root. See `docs/PHASE_2.md` §"PRD-OQ-003". | ~~Phase 2~~ Decided |
| PRD-OQ-004 | **Exact checklist item set.** ~~The items are asserted but not enumerated.~~ **RESOLVED (Phase 5): the full 31-check set of `COMPLIANCE_REQUIREMENTS.md` §12** with stable `rule_key`s, seeded as checklist template v1; a drift guard asserts every key maps to a registered evaluator. See `docs/PHASE_5.md` §"PRD-OQ-004"; AC-P5-13. | ~~Phase 5~~ Decided |
| PRD-OQ-005 | **Required-chapter count.** ~~`mock-data.ts` marks 15 of 18 `required`; template copy says 13. Which are truly required, and does it differ by PBK scope?~~ **RESOLVED (Phase 5): the required set + PBK conditionals** — `manual_template_sections.required` on the seeded system template; seven Bappebti-only checks become NOT_APPLICABLE for `pbkScope = OUT_OF_SCOPE`. See `docs/PHASE_5.md` §"PRD-OQ-005"; AC-P5-13. | ~~Phase 2 / Phase 5~~ Decided |
...
| PRD-OQ-007 | **Publishing authority.** ~~Is publish admin-only, or can a compliance reviewer with `manual:publish` publish directly after approving?~~ **RESOLVED (Phase 6): ADMIN only.** A `COMPLIANCE_REVIEWER` approves the documentation (`COMPLIANCE_REVIEW → APPROVED`); only an `ADMIN` may run `APPROVED → PUBLISHED`. This is an internal Smartin workflow decision, not regulatory approval. See `docs/PHASE_6.md`. | ~~Phase 6~~ Decided |
| PRD-OQ-008 | **Locale strategy.** ~~Separate `manuals` rows per locale, or per-locale versions?~~ **RESOLVED (Phase 2, schema): per-`manuals`-row locale** — `manuals.locale`; one `manuals` row per (EA product, locale); versions locale-agnostic under it. Multi-locale authoring **UI deferred post-MVP** (owner: Product; **target review date 2026-12-31** — a planning checkpoint, not a committed delivery date; not a Phase 8 gate — MVP ships the `id` locale). See `docs/PHASE_8.md` §8B-11. | ~~Post-MVP / Phase 2 schema~~ Decided (schema) / UI deferred, review 2026-12-31 |
| PRD-OQ-009 | **Image scanning.** ~~What performs the `scan_status` check? Unscanned = blocked or warning?~~ **RESOLVED (Phase 5): `scan_status` field stored, no scanner in the MVP, NOT a Phase 5 publish gate.** A real scanner (Supabase extension or external service) is **deferred post-MVP** (owner: Platform/Security; **target review date 2026-12-31** — a planning checkpoint, not a committed delivery date; not a Phase 8 gate — images are org-private via RLS + signed URLs and only enter a published snapshot after a human review round). See `docs/PHASE_5.md` §"PRD-OQ-009"; AC-P5-13. | ~~Phase 2 / Phase 5~~ Decided (MVP) / scanner deferred, review 2026-12-31 |
| PRD-OQ-010 | **Changelog ↔ Bappebti Pasal 8.** ~~How far does the tool go — record the changelog only, or also produce a client-approval artefact and a "report to Kepala Bappebti" reminder/task?~~ **RESOLVED (Phase 6): record + remind only.** For an IN_SCOPE feature/behaviour changelog entry the tool records the entry and shows a Pasal 8 operational reminder (client approval + report to Kepala Bappebti where applicable). It never files, submits, simulates approval, produces a fake client-approval artefact, or marks the obligation complete. OUT_OF_SCOPE: the reminder is not presented as an applicable Indonesian PBK legal requirement. See `docs/PHASE_6.md`. | ~~Phase 6~~ Decided |
| PRD-OQ-011 | **Non-Bappebti EAs.** ~~For an EA only distributed on MQL5 Market (outside Indonesian PBK), which chapters/checks are suppressed vs. still required?~~ **RESOLVED (Phase 5): seven Bappebti-only checklist items become `NOT_APPLICABLE` when `pbkScope = OUT_OF_SCOPE`** (item-level flag in `evaluate.ts`); all other chapters/checks remain required. See `docs/PHASE_5.md` §"PRD-OQ-011"; AC-P5-13. | ~~Phase 5~~ Decided |
| PRD-OQ-012 | **Template editing depth.** ~~Can admins add/remove/reorder chapters in a manual template and change required flags, or only edit titles/help text in v1?~~ **RESOLVED (Phase 2): v1 ships versioned template tables + a seeded system template + template-version recording; NO admin template-CRUD UI.** Per-manual-version custom chapters shipped in Phase 3 at the manual level. An admin template editor (`template:manage`) is **deferred post-MVP** (owner: Product; **target review date 2026-12-31** — a planning checkpoint, not a committed delivery date; not a Phase 8 gate). See `docs/PHASE_2.md` §"PRD-OQ-012". | ~~Phase 2~~ Decided (schema) / admin editor deferred, review 2026-12-31 |
