...
--   (a) the source MUST be the CURRENT latest Manual Version of its lineage (AC-P6-11), verified
--       server-side AFTER a `manuals` row lock so two concurrent clones of the same "latest"
--       cannot both create sibling versions — the second one sees a stale source and is rejected;
--   (b) the RPC resolves the exact CHECKLIST-TEMPLATE version the source Manual Version's result
--       set is bound to (distinct from the *manual* template_id/template_version) and returns it,
--       so the application-side Phase 5 refresh can evaluate the clone against THAT version
--       (fresh states, no copied evidence/overrides). A source with no results yet falls back to
--       the current active checklist template version (documented Phase 5 default).
--
-- New file after 20260901001800 (which is NOT rewritten). Everything else in the clone body is
-- byte-identical to 001800.
...
begin
  ...
  select count(distinct checklist_template_version), min(checklist_template_id), min(checklist_template_version)
    into v_cl_distinct, v_src_cl_tid, v_src_cl_ver
